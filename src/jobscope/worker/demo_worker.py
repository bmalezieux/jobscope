...
from ..scope.get_data import (
    Snapshot, CPUsSnapshot, GPUsSnapshot, ProcessesSnapshot,
    CPUInfo, MemoryLoad, GPUInfo, ProcessInfo
)

def run_demo_agent_loop(output_dir: Path, period: float, 
                        n_nodes: int, n_cpus: int, n_gpus: int):
    """
    Simulate multiple nodes writing snapshots.
    """
    print(f"Starting Demo Agent: {n_nodes} nodes, {n_cpus} CPUs, {n_gpus} GPUs. Output: {output_dir}")
            ...
            for hostname in nodes:
                ...
                for i in range(n_gpus):
                    ...
                    gpus.append(GPUInfo(
                        ...
                        usage_percent=usage,
                        memory_load=MemoryLoad(used_bytes=int(used_vram), total_bytes=total_vram)
                    ))
...
def run_demo_worker(output_dir: Path, period: float, 
                    n_nodes: int = 1, n_cpus: int = 4, n_gpus: int = 1):
    p = Process(target=run_demo_agent_loop, args=(output_dir, period, n_nodes, n_cpus, n_gpus))
    p.start()
    return p
