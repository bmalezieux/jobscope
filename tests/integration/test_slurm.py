...
def slurm_cluster():
    ...
    res = run_command("docker ps --filter name=slurmctld --format '{{.Status}}'", check=False)
    if "Up" not in res.stdout:
        pytest.skip("SLURM cluster not running. Run tests/integration/start_cluster.sh first.")
    
    # Check for sufficient CPU resources
    import multiprocessing
    cpu_count = multiprocessing.cpu_count()
    if cpu_count < 2:
        pytest.skip(f"Ideally we need at least 2 CPUs for testing, found {cpu_count}. Skipping integration tests that require multiple CPUs.")

    yield

def submit_and_monitor(nodes, tasks_per_node, cpus_per_task, gpu_config, workload_script):
    job_id = None
    script_path = None
    ...
    try:
        ...
        cmd = f"docker exec slurmctld sbatch --parsable"
        cmd += f" --nodes={nodes}"
        ...
        started = False
        for i in range(20):
            res = run_command(f"docker exec slurmctld squeue --job {job_id} --noheader --format=%t")
...
@pytest.mark.parametrize("nodes,tasks,cpus,gpus", [
...
"""
    ...
    try:
        ...
        assert reported_cpus == expected_cpus_per_node, f"Expected {expected_cpus_per_node} CPUs, got {reported_cpus}"

    except subprocess.CalledProcessError as e:
        if "Submitted batch job" not in e.stdout and gpus > 0:
            pytest.skip("Could not submit GPU job (likely no GPU configured in cluster)")
        raise e
